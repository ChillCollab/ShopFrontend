...
const Categories = () => {
  ...
  useEffect(() => {
    adminReqs
      ...
      .then((res) => {
        let counter = 1;
        const dataWithId = res.data.map(
          (category: { name: string; image: string; created_at: string; updated_at: string }) => {
      ...
      .catch((e: AxiosError<{ message: string; code: number }>) => {
        if (e.response?.data)
          if (e.response?.data.code !== 306) dispatch(addAlert({ message: e.response?.data.message, type: 'error' }));
      })
      .finally(() => {
        setIsLoading(false);
      });
